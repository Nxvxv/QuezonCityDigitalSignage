...
<body class="relative min-h-screen flex items-center justify-center font-montserrat">
    ...
    var branchLabel = document.getElementById('branch-label');
        ...
        async function handleLoginSubmit(event) {
            ...
            const branches = districtBranchMap[district] ? Array.from(districtBranchMap[district]) : [];
            const branchToSend = (branches.length === 1 && branches[0] === district) ? district : branch;

            try {
                const resp = await fetch('authenticate.php', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ adminName, district, branch: branchToSend })
                });
                const data = await resp.json();
                if (data.success) {
                    // Keep client state for UI convenience
