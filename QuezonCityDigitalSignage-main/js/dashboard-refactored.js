...
function setupEventListeners() {
  ...
  function closeBookModal() {
    ...
    if (bookModal) {
      ...
      document.body.classList.remove('body-blurred');
      // restore default save button label
      const saveBtn = document.getElementById('book-save'); if (saveBtn) saveBtn.textContent = 'Add & Save';
    }
  }
  ...
  function openFooterModal(mode = 'new') {
    if (footerModal) {
      ...
      if (titleEl && submitBtn) {
        if (mode === 'edit') {
          titleEl.textContent = 'Edit Footer Message';
          submitBtn.textContent = 'Update Message';
          submitBtn.classList.remove('bg-green-600');
          submitBtn.classList.add('bg-blue-600','hover:bg-blue-700');
        } else {
          titleEl.textContent = 'New Footer Message';
          submitBtn.textContent = 'Create & Save';
          submitBtn.classList.remove('bg-blue-600','hover:bg-blue-700');
          if (!submitBtn.classList.contains('bg-green-600')) submitBtn.classList.add('bg-green-600');
        }
      }
  ...
  function closeFooterModal() {
    ...
    const hid = document.getElementById('footer-id'); if (hid) hid.value = '';
    // reset labels back to create mode
    const titleEl = document.getElementById('footer-modal-title');
    const submitBtn = document.getElementById('footer-submit-btn');
    if (titleEl) titleEl.textContent = 'New Footer Message';
    if (submitBtn) {
      submitBtn.textContent = 'Create & Save';
      submitBtn.classList.remove('bg-blue-600','hover:bg-blue-700');
      if (!submitBtn.classList.contains('bg-green-600')) submitBtn.classList.add('bg-green-600');
