...
(function () {
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // Function to calculate book card size based on screen width
    function getBookCardSize() {
      const width = window.innerWidth;
      if (width >= 2560) {
        return { width: 260, height: 350, titleSize: '18px', authorSize: '14px', descSize: '15px', categorySize: '14px', postSize: '11px' };
      } else if (width >= 1920) {
        return { width: 180, height: 270, titleSize: '14px', authorSize: '11px', descSize: '10px', categorySize: '9px', postSize: '8px' };
      } else if (width >= 1280) {
        return { width: 130, height: 195, titleSize: '11px', authorSize: '9px', descSize: '8px', categorySize: '7px', postSize: '6px' };
      } else if (width >= 768) {
        return { width: 100, height: 150, titleSize: '12px', authorSize: '9px', descSize: '9px', categorySize: '8px', postSize: '8px' };
      } else {
        return { width: 80, height: 120, titleSize: '11px', authorSize: '8px', descSize: '8px', categorySize: '7px', postSize: '7px' };
      }
    }
        ...
        j.data.forEach(book => {
          ...
          if (isSignagePage) {
            ...
            const coverImg = book.cover_image 
              ...
              : `data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="${size.width}" height="${size.height}"><rect fill="%23ddd" width="${size.width}" height="${size.height}"/><text x="${size.width/2}" y="${size.height/2}" text-anchor="middle" fill="%23999" font-size="12">No Cover</text></svg>`;
              ...
              ">
                ...
                ">
                  <img src="${coverImg}" alt="${escapeHtml(book.title)}" 
                    ...
                    onerror="this.src='data:image/svg+xml;utf8,<svg xmlns=%22http://www.w3.org/2000/svg%22 width=%22${size.width}%22 height=%22${size.height}%22><rect fill=%22%23ddd%22 width=%22${size.width}%22 height=%22${size.height}%22/><text x=%22${size.width/2}%22 y=%22${size.height/2}%22 text-anchor=%22middle%22 fill=%22%23999%22>No Cover</text></svg>'" />
                </div>
                ...
                ">
                  ...
                  <div class="book-title-section" style="flex-shrink: 0;">
                    <div class="book-title" style="font-weight: bold; font-size: ${size.titleSize}; margin-bottom: 4px; line-height: 1.25; word-break: break-word; text-shadow: 2px 2px 4px rgba(0,0,0,0.3);">
                      ${escapeHtml(book.title)}
                    </div>
                    <div class="book-author" style="font-size: ${size.authorSize}; margin-bottom: 2px; line-height: 1.2; opacity: 0.9; font-style: italic; text-shadow: 1px 1px 2px rgba(0,0,0,0.3);">
                      by ${escapeHtml(book.author)}
                    </div>
                    ${isSignagePage && (book.created_by_name || book.created_by) ? `<div style="font-size:${size.postSize}; opacity:0.8;">posted by ${escapeHtml(book.created_by_name || (''+book.created_by))}</div>` : ''}
                  </div>
                  
                  <!-- Description section -->
                  <div class="book-description" style="flex: 1; display: flex; align-items: center; justify-content: center; margin: 8px 0;">
                    <div style="font-size: ${size.descSize}; line-height: 1.4; max-height: 100px; overflow-y: auto; text-align: center; text-shadow: 1px 1px 2px rgba(0,0,0,0.3); word-break: break-word;">
                  ...
                  <div class="book-category" style="flex-shrink: 0;">
                    <div style="font-size: ${size.categorySize}; line-height: 1.2; opacity: 0.8; background: rgba(0,0,0,0.2); padding: 4px 10px; border-radius: 10px; text-shadow: 1px 1px 2px rgba(0,0,0,0.3);">
    ...
    // Add resize listener to reload books when screen width changes (especially for 2560px screens)
